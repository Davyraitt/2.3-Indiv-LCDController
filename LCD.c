...
// Queue so we can queue the lcd write requests, making it "thread safe"
...
//Writer task for writing to the LCD
...
//Set menu, we pass an array menuItems which contains menu items (like play, pause)
void setMenu(MenuItem menuItems[], int size, char descrLineEN[],  char descrLineNL[]) {
    currentItem = 0; // Resetting curentpos to 0
    itemListSize = size; // Defining our new item list size
    itemListPtr = &menuItems[0]; // Setting the pointer

    for (int i = 1; i < 3; ++i) { // Clear the screen
        clearLine(i);
    }
    ...
    writeToLine("         ^         ", 3); // Writing the pointer
...
void scrollRight() {
    //Scrolls the screen one menu item to the right
...
void scrollLeft() {
    //Scrolls the screen one menu item to the left
    if (currentItem != 0) {
        currentItem = currentItem - 1;
        itemListPtr--;
        clearLine(2);
...
void getSelectedOption() {
    // Gets the current selected option, for example "Play"
    // We then redirect to the Play menu by using the correct pointer
    char text[50] = "";
